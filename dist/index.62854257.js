...
window.onscroll = function() {
    ...
    const fadeTime = 250;
    if (scrolled === 0) hideHeader.style.top = "0";
    else if (scrolled > 0) hideHeader.style.top = "-100vh";
    if (scrolled > 200) {
...
const pokemonRepository = (()=>{
    ...
    function add(pokemon) {
        if (typeof pokemon !== "object") console.log("The new Pokémon should have the form of an object");
        else if (!("name" in pokemon && "detailsUrl" in pokemon)) console.log("The object should have the following properties: name, detailsUrl");
    ...
    async function loadDetails(itemDetails) {
        ...
        try {
            ...
            const data = await response.json();
            const { sprites , abilities , height , types , weight ,  } = data;
            pokemon.imageUrl = sprites.other.dream_world.front_default;
            pokemon.abilities = [];
            for(let i = 0; i < abilities.length; i++)pokemon.abilities.push(` ${abilities[i].ability.name}`);
            pokemon.height = height;
            pokemon.types = [];
            for(let i1 = 0; i1 < types.length; i1++)pokemon.types.push(` ${types[i1].type.name}`);
    ...
    function showPopup(item) {
      ...
      `;
        ...
        window.addEventListener("keydown", (e)=>{
            if (e.key === "Escape" && popupContainer.classList.contains("is-visible")) hidePopup();
        });
        window.addEventListener("click", (e)=>{
            const { target  } = e;
            if (target === popupContainer) hidePopup();
        });
    }
    // Hide popup
    function hidePopup() {
...
function search(reset) {
    ...
    for(let i = 0; i < allPokemon.length; i++)if (allPokemon[i].innerText.toLowerCase().includes(input)) allPokemon[i].classList.remove("hide");
    else allPokemon[i].classList.add("hide");
}

//# sourceMappingURL=index.62854257.js.map
